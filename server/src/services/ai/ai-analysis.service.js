  ...
  const itemText = items
    .map((item, index) =>
      [
        ...
        item.replyToCommentText ? `   显式回复的评论：${item.replyToCommentText}` : '',
        item.replyToCommentAuthor ? `   显式回复对象：${item.replyToCommentAuthor}` : '',
...
const normalizeExtensionItems = (items) => {
  ...
  return items
    ...
    .map((item) => ({
      ...
      replyToCommentAuthor: normalizeText(item?.replyToCommentAuthor, 80),
      replyToCommentText: normalizeText(item?.replyToCommentText, 240),
      commentCount: normalizeCount(item?.commentCount),
      repostCount: normalizeCount(item?.repostCount),
      likeCount: normalizeCount(item?.likeCount),
